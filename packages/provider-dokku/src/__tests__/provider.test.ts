...
describe('DokkuProvider', () => {
  ...
  it('apps.get calls apps:report', async () => {
    mockRun.mockResolvedValueOnce('=====> my-app app information\n       App running:         true')
  ...
  it('env.list calls config:show and returns EnvVar[]', async () => {
    mockRun.mockResolvedValueOnce('=====> my-app env vars\nDATABASE_URL:   postgres://localhost\nPORT:   5000')
  ...
  it('databases.list tries all supported engines', async () => {
    ...
    mockRun
      ...
      .mockResolvedValueOnce('=====> my-db info\n       Status:              running\n       Version:             postgres:16.1') // postgres:info
  ...
  it('databases.create calls <engine>:create then <engine>:info', async () => {
    mockRun
      ...
      .mockResolvedValueOnce('=====> new-db info\n       Status:              running\n       Version:             postgres:16') // postgres:info
