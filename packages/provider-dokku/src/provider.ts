...
import { NotFoundError, UnsupportedError, ValidationError } from "@paasman/core";
import { toApp, toDatabase, toEnvVar, toLogLine } from "./normalizers.js";
import {
	parseAppReport,
	parseAppsList,
	parseConfigExport,
...
function validateDokkuName(name: string, label: string): string {
	if (!/^[a-z0-9][a-z0-9-]*$/.test(name)) {
		throw new ValidationError(
			`Invalid ${label}: '${name}'. Only lowercase letters, digits, and hyphens allowed.`,
		);
	}
	return name;
}
...
function validateEnvKey(key: string): string {
	if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
		throw new ValidationError(
			`Invalid env key: '${key}'. Only letters, digits, and underscores allowed.`,
		);
	}
	return key;
}
...
export class DokkuProvider implements PaasProvider {
	...
	env: EnvOperations = {
		...
		delete: async (appId: string, key: string): Promise<void> => {
			await this.sshClient.run(
				`config:unset ${validateDokkuName(appId, "app name")} ${validateEnvKey(key)}`,
			);
		},

		pull: async (appId: string): Promise<Record<string, string>> => {
			const output = await this.sshClient.run(
				`config:export ${validateDokkuName(appId, "app name")}`,
			);
