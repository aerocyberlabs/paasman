...
export function toApp(raw: Record<string, unknown>): App {
	...
	const domains = rawDomains ? rawDomains.map((d) => d.host as string).filter(Boolean) : [];
...
export function toDatabase(raw: Record<string, unknown>): Database {
	const status = mapAppStatus(
		(raw.databaseStatus ?? raw.applicationStatus ?? raw.status) as string,
	);
