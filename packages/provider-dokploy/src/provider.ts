...
export class DokployProvider implements PaasProvider {
	...
	private async *getLogs(id: string, opts?: LogOpts): AsyncGenerator<LogLine> {
		const data = await this.client.post<{ logs: string[] | string }>("/api/application.readLogs", {
			applicationId: id,
	...
	apps: AppOperations = {
		...
		get: async (id: string): Promise<App> => {
			const data = await this.client.get<Record<string, unknown>>("/api/application.one", {
				applicationId: id,
			});
			return toApp(data);
		},
		...
		deploy: async (id: string, _opts?: DeployOpts): Promise<Deployment> => {
			const data = await this.client.post<Record<string, unknown>>("/api/application.deploy", {
				applicationId: id,
			});
	...
	servers: ServerOperations = {
		...
		get: async (id: string): Promise<Server> => {
			const data = await this.client.get<Record<string, unknown>>("/api/server.one", {
				serverId: id,
			});
	...
	databases: DatabaseOperations = {
		...
		get: async (id: string): Promise<Database> => {
			...
			const data = await this.client.get<Record<string, unknown>>("/api/postgres.one", {
				databaseId: id,
			});
		...
		create: async (input: CreateDatabaseInput): Promise<Database> => {
			...
			const data = await this.client.post<Record<string, unknown>>(`/api/${router}.create`, body);
			return toDatabase({
				...data,
	...
	deployments: DeploymentOperations = {
		list: async (appId?: string): Promise<Deployment[]> => {
			...
			const data = await this.client.get<Record<string, unknown>[]>("/api/deployment.all", {
				applicationId: appId,
			});
		...
		get: async (id: string): Promise<Deployment> => {
			...
			const data = await this.client.get<Record<string, unknown>>("/api/deployment.one", {
				deploymentId: id,
			});
			return toDeployment(data);
		},
