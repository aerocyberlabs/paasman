...
describe('CapRoverClient', () => {
  ...
  it('unwraps data from CapRover response envelope', async () => {
    ...
    const result = await client.get<{ appDefinitions: unknown[] }>('/api/v2/user/apps/appDefinitions')
    expect(result.appDefinitions).toHaveLength(1)
  })

  it('strips trailing slashes from base URL', async () => {
    const clientWithSlash = new CapRoverClient({ baseUrl: 'https://captain.example.com/', password: 'pass' })
