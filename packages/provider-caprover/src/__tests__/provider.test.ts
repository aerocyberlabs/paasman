...
describe("CapRoverProvider", () => {
	...
	it("apps.get throws when app not found", async () => {
		...
		await expect(provider.apps.get("nonexistent")).rejects.toThrow(
	...
	it("apps.create registers app and returns it", async () => {
		...
		expect(mockClient.post).toHaveBeenCalledWith("/api/v2/user/apps/appDefinitions/register", {
			appName: "new-app",
	...
	it("apps.create sets env vars if provided", async () => {
		...
		expect(mockClient.post).toHaveBeenCalledWith("/api/v2/user/apps/appDefinitions/update", {
			appName: "new-app",
	...
	it("apps.delete sends delete request", async () => {
		...
		expect(mockClient.post).toHaveBeenCalledWith("/api/v2/user/apps/appDefinitions/delete", {
			appName: "my-app",
		});
	});
	...
	it("servers.get throws when server not found", async () => {
		...
		await expect(provider.servers!.get("nonexistent")).rejects.toThrow(
