...
export function createMigrationPlan(
	...
	if (app.meta.buildPack) {
		warnings.push(
			`Source uses build pack '${app.meta.buildPack}', target may not support it`,
		)
	}
	...
	if (app.meta.image) {
		...
		warnings.push('Could not determine app source — you may need to configure it manually on the target')
