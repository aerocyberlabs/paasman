...
describe("createMigrationPlan", () => {
	...
	it("warns when source and target profiles are the same", () => {
		...
		expect(plan.warnings).toContain("Source and target profiles are the same — are you sure?");
	});
	...
	it("warns about build pack compatibility", () => {
		...
		expect(plan.warnings).toContain("Source uses build pack 'nixpacks', target may not support it");
	});
	...
	it("warns about domains needing DNS reconfiguration", () => {
		...
		expect(plan.warnings).toContain("Domains will need manual DNS reconfiguration");
	});
	...
	it("does not warn about domains when app has no domains", () => {
		...
		expect(plan.warnings).not.toContain("Domains will need manual DNS reconfiguration");
	...
	it("does not warn about build pack when none is set", () => {
		...
		const buildPackWarnings = plan.warnings.filter((w) => w.includes("build pack"));
