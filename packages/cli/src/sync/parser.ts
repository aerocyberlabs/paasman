...
export function parsePaasmanYaml(filePath: string): PaasmanYaml {
	...
	if (raw.apps && typeof raw.apps === "object") {
		for (const [name, rawApp] of Object.entries(
			raw.apps as Record<string, Record<string, unknown>>,
		)) {
	...
	if (raw.databases && typeof raw.databases === "object") {
		for (const [name, rawDb] of Object.entries(
			raw.databases as Record<string, Record<string, unknown>>,
		)) {
...
function parseAppConfig(name: string, raw: Record<string, unknown>): AppConfig {
	...
	const domains = raw.domains ? (raw.domains as string[]).map(String) : undefined;
