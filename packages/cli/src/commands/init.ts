	...
	const cmd = new Command('init')
		...
		.action(async () => {
			...
			if (existsSync(configPath)) {
				...
				const existingProfiles = Object.keys((existingConfig.profiles ?? {}) as Record<string, unknown>)
			...
			if (existingConfig) {
				...
				if (profiles[profileName]) {
					const overwrite = await confirm({ message: `Profile '${profileName}' already exists. Overwrite?`, default: false })
					if (!overwrite) {
						console.log('Aborted.')
						return
					}
