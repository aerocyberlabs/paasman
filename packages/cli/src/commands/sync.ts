...
import { type CurrentState, computeSyncPlan, planIsEmpty } from "../sync/differ.js";
...
function printPlan(plan: ReturnType<typeof computeSyncPlan>, prune: boolean | undefined): void {
	const hasApps =
	...
	if (hasApps) {
		...
		for (const { name, config } of plan.apps.create) {
			...
			if (config.source.type === "git") {
				console.log(
					`    source: git ${config.source.repository} @ ${config.source.branch ?? "main"}`,
				);
			} else {
	...
	if (hasDbs) {
		...
		for (const { name, config } of plan.databases.create) {
			console.log(
				chalk.green(
					`  + ${name} (${config.engine}${config.version ? ` ${config.version}` : ""}, will be created)`,
				),
			);
		}
