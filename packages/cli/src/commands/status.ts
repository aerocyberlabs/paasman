...
import { type ProfileConfig, loadConfig } from "../config.js";
...
export async function createProvider(profile: ProfileConfig): Promise<PaasProvider | null> {
	try {
		...
		const provider = new (
			ProviderClass as new (config: { baseUrl: string; token: string }) => PaasProvider
		)({
			baseUrl: profile.url,
			token: profile.token,
		});
	...
	const cmd = new Command("status")
		...
		.action(async () => {
			...
			for (const [name, profile] of Object.entries(config.profiles)) {
				...
				if (!provider) {
					table.push([name, profile.provider, profile.url, chalk.red("unreachable"), "-"]);
					continue;
				}
