import { type WebhookFormat, formatPayload } from "./formatters.js";
...
export class WebhookManager {
	...
	async notify(event: WebhookEvent): Promise<void> {
		...
		const results = await Promise.allSettled(matching.map((config) => this.send(config, event)));
	...
	private async send(config: WebhookConfig, event: WebhookEvent): Promise<void> {
		...
		if (!response.ok) {
			throw new Error(`Webhook ${config.url} returned ${response.status}: ${response.statusText}`);
		}
	}
}
